...
Copy your unique mock URL from `webhook.site` and register it using Postman or cURL:
...
## Testing the Edge Cases (Retries & Ordering)

- **Test Retry Backoff**: Register a partner with a broken URL (e.g., `http://localhost:9999/fail`). Ingest an event and check the React dashboard. You'll see it actively fail and schedule a retry automatically (10s, 30s, 2m, etc).
- **Test Strict Ordering**: While that first event is stuck in the retry loop you just created, ingest a second event for the *same* `partnerId`. The system will queue the second event and completely refuse to attempt delivery until the first one handles (this guarantees strict sequential delivery).
